...
> **This is the `0.1-poc` version of this reference.** `0.1` (the compiler
> frontend + bytecode VM in `compiler/`/`vm/`) reached feature parity with
> `0.1-poc` as of Phase 9 (`docs/PLAN-0.1.md`) — see **`docs/LANGUAGE.md`**
> for the current, actively-maintained language reference. This file is
> kept as the accurate reference for the `poc/` tree-walk interpreter
> specifically, which stays in the repo as a working reference
> implementation (`CLAUDE.md`) — not retroactively rewritten to describe
> `0.1`. Future versions get their own `LANGUAGE-<version>.md` the same
> way, each superseded file staying put as its version's reference rather
> than being deleted.
...
`docs/PLAN-0.1.md` for the `0.1` implementation (compiler frontend in F#,
bytecode VM backend in C++23) that now supersedes this one as the primary
reference.
