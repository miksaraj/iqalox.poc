...
> **This is the `0.1` version of this reference.** `0.2` (which adds
> vector indexing, lambdas, cons/comprehensions, vector-literal spread, an
> array/matrix standard library, `pub`/`mut` property and method
> visibility, and mixin/trait composition on top of everything below) is
> now the current, actively-maintained version — see **`docs/LANGUAGE.md`**.
> This file is kept as the accurate reference for `0.1` specifically,
> which stays a real, buildable milestone in this repository's history —
> not retroactively rewritten to describe `0.2`. For the even older
> `0.1-poc` tree-walk interpreter, see `docs/LANGUAGE-POC.md`. Each
> version gets its own `LANGUAGE-<version>.md` this way as Iqalox evolves,
> each superseded file staying put as its version's reference rather than
> being deleted.
...
programs live under `langspec/versions/0.1/examples/*.iqx` (this
version's own frozen fixture snapshot, `docs/PLAN-0.2.md` decision 13 —
not the current, `0.2`-syntax top-level `langspec/examples/`).
`scripts/conformance-test.sh` (`docs/PLAN-0.1.md` Phase 9), which diffed
these fixtures' output against `poc/` byte-for-byte, was retired during
`0.2` Phase 7 (`docs/PLAN-0.2.md`): decisions 8-11's breaking changes to
the object model mean `compiler/`+`vm/` (now `0.2`) can no longer run
`poc/`-era class fixtures at all, and the repository owner's explicit
call was to drop cross-implementation conformance testing entirely rather
than maintain it — pre-`1.0`, backward compatibility with an earlier
version isn't a goal.
...
Item 12 of this list as it stood while this file was still the live,
current `docs/LANGUAGE.md` — a `[line N]` misattribution for runtime
errors raised from inside a `map`/`filter`/`reduce`/`sort` prelude
function — has been **removed from this frozen snapshot**: those
functions are a `0.2` addition (`docs/PLAN-0.2.md` Phase 5) with no
existence in `0.1` at all, so a limitation that only manifests when
calling them doesn't actually describe `0.1`. See `docs/LANGUAGE.md`
§13 for the `0.2`-accurate version of this item.
